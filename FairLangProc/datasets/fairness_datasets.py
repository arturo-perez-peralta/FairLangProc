...
def tgzReader(path: str) -> Optional[str]:
    """
    Extracts a .tgz file and returns the path to the extraction directory.
    
    Checks if the target directory exists and is not empty to avoid re-extraction.
    """
    if not os.path.exists(path):
        print(f"File not found: {path}")
        return None
    
    # Create an output directory name by removing the .tgz extension
    output_path = path.replace('.tgz', '')
    ...
    try:
        ...
        if not os.path.exists(output_path) or not os.listdir(output_path):
            ...
            print(f"Directory '{output_path}' not empty. Assuming already extracted.")
        return output_path
...
def RealToxicityPromptsHandler() -> Optional[pd.DataFrame]:
    """
    Handle RealToxicityPrompts dataset.
    This dataset contains prompts designed to elicit toxic language from models.
    """
    path = obtainPath('RealToxicityPrompts')
    file_path = os.path.join(path, 'prompts.jsonl')
    if not os.path.exists(file_path):
        print(f"Data file not found at {file_path}")
        print("Please download 'prompts.jsonl' and place it in the 'data/RealToxicityPrompts' directory.")
        return None
    return jsonlReader(file_path)

def RedditBiasHandler(data: str = '') -> Union[pd.DataFrame, Dict[str, Any], None]:
    """Handle RedditBias dataset"""
    path = obtainPath('RedditBias')
    if data.lower() in ('', 'h', 'help'):
        print('List of available datasets:')
        if os.path.exists(path):
            for dataset in os.listdir(path):
...
def BiasNLIHandler(data: str = '') -> Union[pd.DataFrame, Dict[str, Any], None]:
    """Handle Bias-NLI dataset"""
    path = os.path.join(obtainPath('Bias-NLI'), 'files')
    
    if not os.path.exists(path):
        print("Files for Bias-NLI not generated. Please run the process first.")
        return None
...
def BiasInBiosHandler(data: str = '') -> Optional[pd.DataFrame]:
    """
    Handle Bias in Bios dataset.
    This dataset is used to measure gender bias in occupation classification.
    The data is expected to be a pickled pandas DataFrame.
    """
    path = obtainPath('BiasInBios')
    file_path = os.path.join(path, 'BIOS.pkl')
    if os.path.exists(file_path):
        try:
            # The original dataset is distributed as a pickle file
        ...
        print(f"Data file not found at {file_path}")
        print("Please download the 'BIOS.pkl' file and place it in the 'data/BiasInBios' directory.")
        return None

def HONESTHandler() -> Optional[pd.DataFrame]:
    """Handle HONEST dataset"""
    file_path = os.path.join(obtainPath('HONEST'), 'honest.csv')
    return csvReader(file_path) if os.path.exists(file_path) else None

def PANDAHandler(data: str = '') -> Union[pd.DataFrame, Dict[str, Any], None]:
    """Handle PANDA dataset"""
    path = obtainPath('PANDA')
    if data.lower() in ('', 'h', 'help'):
        print('List of available datasets:')
        if os.path.exists(path):
            for dataset in os.listdir(path):
...
def TrustGPTHandler(data: str = '') -> Union[pd.DataFrame, Dict[str, Any], None]:
    """Handle TrustGPT dataset"""
    path = os.path.join(obtainPath('TrustGPT'), 'files')

    if not os.path.exists(path):
        print("Files for TrustGPT not generated. Please run the process first.")
        return None
...
def UnQoverHandler() -> Optional[pd.DataFrame]:
    """Handle UnQover dataset"""
    file_path = os.path.join(obtainPath('UnQover'), 'unqover.csv')
    return csvReader(file_path) if os.path.exists(file_path) else None

def WinoQueerHandler() -> Optional[pd.DataFrame]:
    """Handle WinoQueer dataset"""
    file_path = os.path.join(obtainPath('WinoQueer'), 'winoqueer.csv')
    return csvReader(file_path) if os.path.exists(file_path) else None
...
def RunProcessAndDownload(name: str, **kwargs) -> Optional[Dict[str, Any]]:
    ...
    if name in need_python:
        ...
        # Run the script to generate data files
